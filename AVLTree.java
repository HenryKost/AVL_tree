...
// so in conclusion this function is O(log(n))
 ...
 //we need to check by 2
   public String min()
   {
	   if(root==null|| !root.isRealNode())return null;
	    AVLNode temp=root;
 ...
 //we need to check by 2
   ...
   */
   ...
   //O(n) because each line is O(1) except keysToArray(root, sortedKeys, index); which is O(n)
  public int[] keysToArray()
  {
  ...
  //O(n) because the function gets array in length n and goes threw every node in the AVL tree once,
  // a recursive function that all function are O(1) that runs for every node once is O(n)
